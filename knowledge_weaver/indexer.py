...
from .ingest import chunk_text, collect_files, read_csv, read_image, read_text
        ...
        for f in files:
            if f.suffix.lower() in [".txt", ".md", ".pdf", ".csv"]:
                ...
                for chunk in chunk_text(txt):
                    ...
                    text_meta.append(
                        {"type": "text", "path": str(f), "chunk": chunk[:2000]}
                    )
            elif f.suffix.lower() in [".png", ".jpg", ".jpeg"]:
                img = read_image(f)
                emb = self.image_model.encode(
